...
Setup:
  1. Drop this file in your cogs/ folder.
  2. In your bot's setup_hook (or on_ready, guarded so it only runs once):
         await bot.load_extension("cogs.role_management")
         bot.add_view(RoleRequestView())   # re-attaches the persistent Accept/Deny buttons
  3. Make sure the bot's top role sits above any role it will be asked to create,
...
class RoleCog(commands.Cog):
    ...
    async def create(self, interaction: discord.Interaction, name: str, hex_code: str):
        ...
        channel = self.bot.get_channel(REQUEST_CHANNEL_ID) or await self.bot.fetch_channel(REQUEST_CHANNEL_ID)
